...
def get_crystal_string_old(atoms):
...
def get_crystal_string(atoms):
    ...
    crystal_str = (
        ...
        + "\n".join(
            [
                str(t) + " " + " ".join(["{0:.1f}".format(x) for x in c])
                for t, c in zip(atom_ids, frac_coords)
            ]
        )
    )
...
def run_atomgpt(
    ...
    model.lm_head = torch.nn.Sequential(torch.nn.Linear( model.config.hidden_size, latent_dim),torch.nn.Linear( latent_dim, 1) )  
