...
def NotebookProgressCallback_on_train_begin(Trainer_metrics):
    def _NotebookProgressCallback_on_train_begin(
...
def NotebookProgressCallback_on_log(Trainer_metrics):
    def _NotebookProgressCallback_on_log(
            ...
            if len(self.inner_table) > 1:
                ...
                if last_values[0] != values[first_column]:
                    ...
                    self.inner_table.append(
                        [
                            values[c] if c in values else "No Log"
                            for c in columns
                        ]
                    )
                else:
                ...
                self.inner_table.append(
                    [values[c] if c in values else 0 for c in columns]
                )
            pass
        pass
...
def get_trl_metrics():
    ...
    for trainer in trainers:
        ...
        metrics_f = re.findall(
            r"metrics\[f[\"\']\{[^\}]{1,}\}([^\"\']{1,})[\"\']\]", file
        )
        stats_f = re.findall(
            r"stats\[f[\"\']\{[^\}]{1,}\}([^\"\']{1,})[\"\']\]", file
        )
        ...
        left_prefix = (
            'prefix = "eval_" if train_eval == "eval" else ""' in file
        )
        if left_prefix:
            metrics += metrics_f
...
def PatchRLStatistics(algorithm="grpo_trainer", other_metrics=[]):
    ...
    if algorithm not in all_metrics:
        print(
            f"AtomGPT for {algorithm.upper()} is not yet implemented! Just ignore this function.\n"
