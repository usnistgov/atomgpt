...
import re
from transformers.models.llama.modeling_llama import logger
from atomgpt.inverse_models.tokenizer_utils import fix_sentencepiece_gguf
...
def unsloth_save_model(
    ...
    if (
        (save_method == "lora") or (save_method == "merged_4bit")
    ) and push_to_hub:
        if token is None:
            raise RuntimeError(
        ...
        if hasattr(model, "config"):
            print(
                f"Saved {save_method} model to https://huggingface.co/"
                + save_directory
            )
        pass
    ...
    if (
        ...
        or (
            ...
            or not hasattr(internal_model.model, "layers")
        )
    ):
        # Do general saving
        ...
        for item in LLAMA_WEIGHTS:
            ...
            #     max_ram = max(max_ram - W.nbytes, 0)
                ...
                state_dict[name] = torch.load(
                    filename, map_location="cpu", mmap=True
                )
        pass
    ...
    if save_pretrained_settings["push_to_hub"] and (
...
def install_llama_cpp_old(version=-10):
    ...
    if os.path.exists("llama.cpp"):
        ...
        for i in range(10):
            print(
                f"**[WARNING]** Deleting llama.cpp directory... {10-i} seconds left."
            )
            time.sleep(1)
        import shutil
    ...
    for command in commands:
        with subprocess.Popen(
            ...
            for line in sp.stdout:
                print(
                    line.decode("utf-8", errors="replace"), flush=True, end=""
                )
        pass
    pass
    # Check if successful
...
def _fix_gemma_gguf():
    ...
    gemma_end = text.find(
        b"self.gguf_writer.add_tensor(new_name, data)", gemma_start
    )
    if gemma_end == -1:
        return
    ...
    gemma_text = (
        gemma_text[:find_bad]
        + good_text
        + gemma_text[find_bad + len(bad_text) :]
    )
...
def save_to_gguf(
    ...
    print_info = (
        ...
        f"[1] Converting HF to GUUF 16bits will take 3 minutes.\n"
        f"[2] Converting GGUF 16bits to {quantization_method} will take 20 minutes.\n"
        f"In total, you will have to wait around 26 minutes.\n"
    )
    print(print_info)
    ...
    final_location = (
        f"./{model_directory}-unsloth.{first_conversion.upper()}.gguf"
    )
...
def upload_to_huggingface(
    ...
    save_directory, username = _determine_username(
        save_directory, old_username, token
    )
    ...
    if not os.path.exists("llama.cpp"):
        ...
        try:
            ...
            if IS_KAGGLE_ENVIRONMENT:
                ...
                python_install.wait()
                install_llama_cpp_blocking(use_cuda=False)
                new_save_directory, old_username = unsloth_save_model(
                    **arguments
                )
                makefile = None
            else:
