...
from atomgpt.inverse_models.patching_utils import patch_model_and_tokenizer
...
if importlib.util.find_spec("vllm") is not None:
    ...
    if not hasattr(
        ...
        def _apply_4bit_weight(
            ...
            out_dim_1 = sum(
                [
                    quant_state[1].shape[0]
                    for quant_state in quant_states.items()
                ]
            )
            out = torch.empty(
            ...
            for i in range(len(quant_states)):
                ...
                out[:, current_index : current_index + output_size] = (
                    matmul_4bit(
                        bf_x,
                        qweight[offsets[i] : offsets[i + 1]].t(),
                        quant_states[i],
                    )
                )
    ...
    def patch_vllm_bitsandbytes():
        ...
        vllm.model_executor.layers.quantization.bitsandbytes.BitsAndBytesLinearMethod._apply_4bit_weight = (
            _apply_4bit_weight
        )
        ...
        vllm_config_logger.addFilter(
            HideLoggingMessage("is not fully optimized")
        )
    ...
    def patch_vllm_lora_tokenizer():
        ...
        vllm.transformers_utils.tokenizer.get_lora_tokenizer_async = (
            _return_nothing
        )
        ...
        try:
            ...
            vllm.transformers_utils.tokenizer_group.tokenizer_group.get_lora_tokenizer = (
                _return_nothing
            )
        ...
        try:
            ...
            vllm.transformers_utils.tokenizer_group.TokenizerGroup.get_lora_tokenizer = (
...
if importlib.util.find_spec("bitsandbytes") is not None:
    ...
    def from_dict(
        ...
        if not len(qs_key) and "quant_type" not in qs_dict:
            ...
            len(qs_key) != 1
            or qs_key[0].split(".")[-1] not in cls.valid_qs_type_keys
        ):
            raise ValueError(
        ...
        qs_dict = {
            k.split(".")[-1]: v for k, v in qs_dict.items()
        }  # strip prefixes
...
def vllm_dynamic_quant_supported(
    ...
    llm_int8_skip_modules = config.quantization_config.get(
...
def get_vllm_state_dict(llm, return_state_dict=False, config=None):
    ...
    def get_state_dict(prefix, kk, state_dict, proj):
        ...
        if hasattr(qweight, "bnb_quant_state"):
            ...
            state_dict[prefix + ".weight"] = qweight[
                offsets[kk] : offsets[kk + 1]
            ]
            quant_state_dict[prefix + ".weight.quant_state"] = quant_states[kk]
            quant_state_dict[prefix + ".weight"] = state_dict[
                prefix + ".weight"
            ]
            ...
            # Normal FP16 weights
            qweight.requires_grad_(
                False
            )  # Disable grad - sometimes vLLM forgets
            state_dict[prefix + ".weight"] = qweight[
                dim_offsets[kk] : dim_offsets[kk + 1]
            ]
            quant_state_dict[prefix + ".weight"] = state_dict[
                prefix + ".weight"
            ]
    ...
    embed_tokens = getattr(
        embed_tokens, "base_layer", embed_tokens
    ).weight.data

    # Counteract vLLM padding vocabs for LoRA
    if vocab_size is not None:
        embed_tokens = embed_tokens[:vocab_size]
    state_dict["model.embed_tokens.weight"] = embed_tokens
    quant_state_dict["model.embed_tokens.weight"] = state_dict[
    ...
    for kk in range(len(vllm_internals.model.layers)):
        ...
        get_state_dict(
            f"model.layers.{kk}.self_attn.q_proj", 0, state_dict, proj
        )
        get_state_dict(
            f"model.layers.{kk}.self_attn.k_proj", 1, state_dict, proj
        )
        get_state_dict(
            f"model.layers.{kk}.self_attn.v_proj", 2, state_dict, proj
        )

        proj = vllm_internals.model.layers[kk].self_attn.o_proj
        get_state_dict(
            f"model.layers.{kk}.self_attn.o_proj", 0, state_dict, proj
        )
    ...
    for key in old_state_dict:
        try:
            torch.testing.assert_close(
                old_state_dict[key], new_state_dict[key], check_stride=True
            )
...
def convert_vllm_to_huggingface(
    ...
    if quantization_config != {} or bnb_config is not None:
        ...
        # Get bnb_config flags
            ...
            kwargs["quant_storage"] = _get_dtype(
                bnb_config.bnb_4bit_quant_storage
            )
    ...
    for kk in range(config.num_hidden_layers):
        for layer_name in layer_names:
            ...
            if f"{layer_name}.weight.quant_state" in quant_state_dict:
                # Layer is quantized!
                quant_state = quant_state_dict[
                    f"{layer_name}.weight.quant_state"
                ]
                ...
                layer.weight = Params4bit(
                    data=weight, requires_grad=False, **kwargs
                )
...
def approximate_vllm_memory_usage(
    ...
    lm_head = (
        0 if getattr(config, "tie_word_embeddings", True) else vocab_size * hd
    )
    ...
    bytes_for_model = (
        total_quantizable_elements / factor
        + total_float16_elements
        + lora_elements
    )
    ...
    max_num_batched_tokens = int(
        0.95 * (memory_left_for_kv_cache / kv_elements)
    )
    # Round by 256
...
def load_vllm(
    ...
    use_bitsandbytes = use_bitsandbytes or model_name.lower().endswith(
        "-bnb-4bit"
    )
...
def create_batches(requests, num_sequences=64):
    ...
    batches = [
        requests[offsets[i] : offsets[i + 1]] for i in range(len(offsets) - 1)
    ]
    return batches
...
def prepare_vllm_lora_loading(model):
    ...
    for v_layer, m_layer in zip(
        vllm_model.model.layers, model.model.model.layers
    ):
...
def convert_lora_modules(
    ...
    if (hasattr(model, "peft_config") and "default" in model.peft_config) and (
        ...
        state_dict = {
            ...
            if (v.dtype != dtype)
            and (".lora_A.default" in k or ".lora_B.default" in k)
        }
        if len(state_dict) == 0:
            return {}
...
def _test_same_model(model, new_model, input_ids):
    ...
    new_rotary = new_model.model.rotary_emb.__class__(
        new_model.config, device="cuda"
    )
    ...
    for i, (old, new) in enumerate(
        zip(model.model.layers, new_model.model.layers)
    ):
        print(i, end=",")
        residualA = A
        residualB = B
        ...
        torch.testing.assert_close(
            df(old.self_attn.q_proj), df(new.self_attn.q_proj)
        )
        torch.testing.assert_close(
            df(old.self_attn.k_proj), df(new.self_attn.k_proj)
        )
        torch.testing.assert_close(
            df(old.self_attn.v_proj), df(new.self_attn.v_proj)
        )
        ...
        residualB = B
        torch.testing.assert_close(
            old.post_attention_layernorm.weight,
...
def _test_get_vllm_state_dict(
    ...
    messages = [
        ...
        [
            {
                "role": "user",
                "content": "Explain Newton's third law of motion.",
            },
        ],
        [
    ...
    input_ids = tokenizer(
        inputs[0], add_special_tokens=False, return_tensors="pt"
    )
