...
#    patch_tokenizer as _patch_tokenizer,
# )
from atomgpt.inverse_models.patching_utils import (
    patch_compiling_bitsandbytes,
    patch_layernorm,
    patch_torch_compile,
    patch_model_and_tokenizer,
...
from atomgpt.inverse_models.loss_utils import (
    HAS_CUT_CROSS_ENTROPY,
    fused_linear_cross_entropy,
    _unsloth_get_batch_samples,
)
from atomgpt.inverse_models.vision_utils import (
    process_vision_info,
)
from atomgpt.inverse_models.compiler import (
    get_transformers_model_type,
    unsloth_compile_transformers as _unsloth_compile_transformers,
)
from atomgpt.inverse_models.training_utils import (
...
warnings.filterwarnings(
    action="ignore", category=FutureWarning, module="xformers"
)
warnings.filterwarnings(
...
from transformers.training_args import (
    logger as transformers_training_args_logger,
)
...
transformers_training_args_logger.addFilter(
    HideLoggingMessage("torch.distributed")
)
...
try:
    from transformers.modeling_utils import (
        logger as transformers_modeling_utils_logger,
    )

    transformers_modeling_utils_logger.addFilter(
        HideLoggingMessage("ForCausalLMLoss")
    )
...
try:
    from accelerate.utils.modeling import (
        logger as accelerate_utils_modeling_logger,
    )
...
def patch_mistral_nemo_config(config):
    if "head_dim (" not in config:
        ...
        config = config.replace(
            "self.sliding_window = sliding_window", add_head_dim
        )
    pass
    return config
...
for model_name in model_architectures:
    config_filepath = (
        f"transformers.models.{model_name}.configuration_{model_name}"
    )
...
if Version(peft_version) < Version("0.12.0"):
    ...
    try:
        ...
        source = source.replace(
            "def update_layer", "def LoraLayer_update_layer"
        )
        exec(source, globals())
...
length_spaces = len(
    re.match(r"[\s]{1,}", BitsAndBytesConfig__init__[0]).group(0)
)
BitsAndBytesConfig__init__ = "\n".join(
...
def offload_to_disk(
    ...
    file_location = os.path.join(
        temporary_location, model.config._name_or_path
    )
...
def offload_input_embeddings(
    ...
    new_input_embeddings._offloaded_file_location = (
...
def _unsloth_pre_compute_loss(self, model, inputs, *args, **kwargs):
    ...
    if (
        ...
        and getattr(
            getattr(self, "args", self), "gradient_accumulation_steps", 1
        )
        != 1
    ):
...
def patch_gradient_accumulation_fix(Trainer):
    ...
    if hasattr(Trainer, "get_batch_samples"):
        ...
        if (
            ...
            if (
                Trainer.get_batch_samples.__name__
                != "_unsloth_get_batch_samples"
            ):
...
for j, function in enumerate(functions):
    if function.startswith("__") and function.endswith("__"):
        exec(
            f"def raise_{j}(*args, **kwargs): print('{function}')",
            globals(),
            locals(),
        )
        try:
