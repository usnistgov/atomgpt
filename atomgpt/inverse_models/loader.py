...
def _get_model_name(model_name, load_in_4bit=True):
    ...
    if not SUPPORTS_FOURBIT and model_name in INT_TO_FLOAT_MAPPER:
        ...
        load_in_4bit and SUPPORTS_FOURBIT and model_name in FLOAT_TO_INT_MAPPER
    ):
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    f"AtomGPT: `{model_name}` is not a full model or a PEFT model."
                )
        ...
        if model_type == "llama":
            ...
            print("Trying unsupported model model_type", model_type)
            dispatch_model = AutoModelForCausalLM.from_pretrained(model_type)
            raise NotImplementedError(
                f"AtomGPT: {model_name} not supported yet!\n"
        ...
        if is_peft:
            ...
            model = PeftModel.from_pretrained(
                model, old_model_name, token=token
            )
            # Patch it as well!
