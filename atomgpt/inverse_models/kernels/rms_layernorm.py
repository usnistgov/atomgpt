...
class Fast_RMS_Layernorm(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, X: torch.Tensor, W: torch.Tensor, eps: float, gemma: bool = False
    ):
        shape = X.shape
        dim: int = shape[-1]
        X = X.view(-1, dim)
...
class AtomGPT_LlamaRMSNorm(LlamaRMSNorm):
    def forward(self, X):
...
try:
    ...
    class AtomGPT_MllamaTextRMSNorm(MllamaTextRMSNorm):
        def forward(self, X):
            return fast_rms_layernorm(self, X, gemma=False)
