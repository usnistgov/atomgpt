...
def prepare_n_gradient_checkpoints(
    ...
    boundaries = calculate_n_gradient_checkpoints(
        n_layers, layers_per_checkpoint
    )
            ...
            if torch.is_tensor(arg):
                ...
                if i == 0 and arg.requires_grad:
                    ...
                    if (
                        ...
                        and CURRENT_GC_INDEX != LAST_GC_INDEX
                    ):
                        use_gpu_buffer = True
                        global CPU_BUFFERS
                        global GPU_BUFFERS
...
def unpatch_unsloth_smart_gradient_checkpointing():
    ...
    if (
        torch.utils.checkpoint.checkpoint.__name__ == "unsloth_checkpoint"
    ) and hasattr(torch.utils, "_old_checkpoint"):
...
@torch._disable_dynamo
def unsloth_offloaded_gradient_checkpoint(
