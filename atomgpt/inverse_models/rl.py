...
import re
import torch
from atomgpt.inverse_models.compiler import create_new_function
from atomgpt.inverse_models.logging_utils import PatchRLStatistics
from atomgpt.inverse_models.rl_replacements import RL_REPLACEMENTS
from atomgpt.inverse_models.rl_replacements import (
    RL_EXTRA_ARGS,
    RL_FUNCTIONS,
    RL_PRE_ITEMS,
    RL_CONFIG_CHANGES,
...
def _patch_trl_rl_trainers(trainer_file="grpo_trainer"):
    ...
    RLTrainer_source = RLTrainer_replacement.format(
        ...
        RLTrainer_kwargs=",**kwargs"[
            1 if RLTrainer_call_args.endswith(",") else 0 :
        ],
        ...
        RLConfig_kwargs=",**kwargs"[
            1 if RLConfig_call_args.endswith(",") else 0 :
        ],
    ...
    if (
        __RLConfig_doc__ != ""
        and RLTrainer_source.count(__RLTrainer_doc__) == 2
    ):
...
def patch_functions(
    ...
    for function in functions:
        ...
        # Find all imports
        imports += [
            x for x in all_imports if not x.startswith("_") and x in source
        ]
...
def patch_trl_rl_trainers():
    ...
    all_trainers = [
        x for x in all_trainers if x.islower() and x.endswith("_trainer")
    ]
