...
def fix_zero_training_loss(model, tokenizer, train_dataset):
    ...
    if type(row) is dict and "labels" in row:
        ...
        for i, row in enumerate(train_dataset):
            try:
                check_tokens = list(set(row["labels"]))
            except:
                continue
            if len(check_tokens) == 1 and check_tokens[0] == -100:
                seen_bad += 1
            else:
                seen_good += 1
            if i >= 100:
...
def prepare_model_for_training(
    ...
    for name, param in model.named_parameters():
        ...
        if not full_finetuning:
            ...
            if train_embedding and (
                "embed_tokens" in name or "embedding" in name
            ):
                requires_grad = True
    ...
    while hasattr(m, "model"):
        ...
        if use_gradient_checkpointing == True and hasattr(
...
def get_max_steps(training_args, n_training_samples, train_dataset):
    ...
    if max_steps > 0:
        ...
        steps_per_epoch = math.ceil(
            n_training_samples / total_train_batch_size
        )
...
def unsloth_train(trainer):
    ...
    decay_parameters = frozenset(
        Trainer.get_decay_parameter_names(None, model)
    )
    yes_decay, no_decay = [], []
    ...
    total_train_batch_size, max_steps, num_train_epochs = get_max_steps(
        training_args, n_training_samples, trainer.train_dataset
    )
    ...
    with ProgressBar(total=max_steps, dynamic_ncols=True) as progress_bar:
        for epoch in range(num_train_epochs):
            ...
            for j in range(max_iters_per_epoch):
                n_batches = (
                    leftover_ga if j == (max_iters_per_epoch - 1) else ga
                )
                batches = [
                    next(train_dataloader_iterator) for j in range(n_batches)
                ]
                ...
                for batch in batches:
                    ...
                    with autocast_context_manager:
                        loss = model(
                            input_ids=input_ids, labels=labels, n_items=n_items
                        ).loss
                ...
                if step % logging_steps == 0:
                    progress_bar.write(
                        f"{step}, {round(accumulated_loss.cpu().item(), 4)}"
                    )
                pass
    ...
    trainer_stats = Trainer_Stats(
        metrics={"train_runtime": end_time - start_time}
    )
