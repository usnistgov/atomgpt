...
from atomgpt.inverse_models.peft_utils import (
    get_peft_regex,
...
from atomgpt.inverse_models.training_utils import prepare_model_for_training
import types
import functools
import os
import gc
...
class FastBaseModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        auto_processor = (
            AutoProcessor if (is_vlm or is_whisper) else AutoTokenizer
        )
        ...
        if os.environ.get("UNSLOTH_DISABLE_FAST_GENERATION", "0") == "0":
            if model.generate.__name__ != "unsloth_base_fast_generate":
                ...
                unsloth_base_fast_generate.__doc__ = (
                    model._old_generate.__doc__
                )
                model.generate = types.MethodType(
                    unsloth_base_fast_generate, model
                )
        pass
    ...
    def post_patch_model(
        ...
        if (
            _get_dtype(model.config.torch_dtype) == torch.bfloat16
            and full_finetuning
        ):
