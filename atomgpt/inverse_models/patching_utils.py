...
def patch_model_and_tokenizer(
    ...
    while hasattr(m, "model"):
        if hasattr(m, "config"):
            if m.config.torch_dtype == "float32":
                m.config.torch_dtype = torch.float32
            elif m.config.torch_dtype == "bfloat16":
                m.config.torch_dtype = torch.bfloat16
            elif m.config.torch_dtype == "float16":
                m.config.torch_dtype = torch.float16
        pass
        m = m.model
    pass
    if hasattr(m, "config"):
    ...
    for name, module in model.named_modules():
        ...
        if downcast_rope and (
            ...
            if hasattr(module, "cos_cached") and (
                ...
                module.short_cos_cached = module.short_cos_cached.to(
                    correct_dtype
                )
                module.short_sin_cached = module.short_sin_cached.to(
                    correct_dtype
                )
            pass
        pass
    ...
    if tokenizer is not None and len(tokenizer) > old_input_embedding.shape[0]:
        ...
        old_input_embedding.resize_(
            len(tokenizer), old_input_embedding.shape[1]
        )
        old_input_embedding.requires_grad_(requires_grad)

        # Fix up all vocab sizes
        current_model = model
        while hasattr(current_model, "model") and hasattr(
        ...
        if hasattr(current_model, "model") and hasattr(
            current_model, "config"
        ):
            if hasattr(current_model.config, "vocab_size"):
                current_model.config.update({"vocab_size": len(tokenizer)})
    ...
    if old_output_embedding.numel() != 0:
        ...
        lm_head.weight = (
            old_output_embedding if not is_tied else old_input_embedding
        )
...
def check_conversion_mappings(model, current_key_name_str, skip_modules):
    ...
    if any(
        allowed_name in model.__class__.__name__.lower()
        for allowed_name in VLMS
    ):
        if (
            hasattr(model._root_cls, "_checkpoint_conversion_mapping")
            and len(model._root_cls._checkpoint_conversion_mapping) > 0
        ):
            ...
            reverse_conversion_mappings = {
                v: k
                for k, v in model._root_cls._checkpoint_conversion_mapping.items()
            }
            ...
            for pattern, replacement in reverse_conversion_mappings.items():
                try:
                    ...
                    key, n_replace = re.subn(
                        pattern, replacement, current_key_name_str
                    )
...
if hasattr(
    ...
    x = ", ".join(functions)
    exec(
        f"from transformers.integrations.bitsandbytes import ({x})", globals()
    )
